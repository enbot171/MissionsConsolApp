  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-3">
      ...
      {status?.needsReconnect && !status?.connected ? (
        // Amber, not red: Google ends the grant every week or so while the
        // consent screen is unverified. Expected, but the user must act.
        <>
          <div className="rounded-xl border border-amber-100 bg-amber-50 px-3 py-2.5 space-y-1">
            <p className="text-xs font-semibold text-amber-700">Google ended the connection</p>
            <p className="text-[11px] text-amber-600">
              This happens about once a week until the app is verified by Google. Your meetups
              are safe — reconnect to put them back on your calendar.
            </p>
          </div>
          <button
            onClick={connect}
            disabled={busy}
            className="w-full py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-semibold rounded-xl text-sm disabled:opacity-50 transition-colors"
          >
