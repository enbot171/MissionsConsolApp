...
| `archived` | boolean | Soft delete — filtered out of all active queries |
| `createdAt` | timestamp | Used for Daily/Weekly/All Time stats |
| `metOn` | timestamp | When the person was first met |
...
| Function | Description |
|---|---|
| `addPerson(data)` | Creates person with `teamId`, returns new ID |
...
| `/` | `app/page.js` | Home dashboard. Period selector (Daily/Weekly/All Time). My Stats: contacts made, gospel shared, prayed for, salvations + active disciples count. Team Stats: same fields aggregated across all team members + active disciples + active core team count. |
...
| `/add-contact` | `app/add-contact/page.js` | Quick/Full toggle. Accepts URL params `?cgId=` and `?ministry=` to pre-fill CG or Core Team assignment and switch to Full tab automatically. |
...
|---|---|
| `BottomNav` | User app nav: Home · People · FAB(+) · Groups · Settings. Active tab shows blue icon + dot. |
| `AdminBottomNav` | Admin app nav: Home · Users · Teams. Same visual style. |
| `PageShell` | User app page wrapper: fixed header with optional back button + right action slot + `BottomNav` |
...
**My Stats** — from `getPeopleByAssignee(user.uid)`:
```
├── Contacts Made, Gospel Shared, Prayed For, Salvations (period-filtered)
└── Active Disciples (all-time count, not period-filtered)
```

**Team Stats** — aggregated from all users in the team:
```
fetch getUsersByTeam(teamId)
...
**Limitation:** Period stats count contacts *created* in the period with flags already set. Toggling a flag later on an older contact won't appear in period stats. Fix would require `gospelSharedAt`, `prayedAt`, `savedAt` timestamps.
